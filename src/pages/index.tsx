...
function getBoardId(query: GetServerSidePropsContext['query']) {
  const boardId = query?.board_id as string;
  console.log('Board ID:', boardId);

  return boardId ?? '521784375517072934';
}
...
export async function getServerSideProps({ query }: GetServerSidePropsContext) {
  ...
  const board_id = getBoardId(query);

  const searchParams = new URLSearchParams();

  // searchParams.append('source_url', board_url);
  searchParams.append('data', JSON.stringify({
    options: {
      board_id,
      // board_url,
      currentFilter: -1,
  ...
  searchParams.append('_', String(Date.now()));

  const url = 'https://www.pinterest.de/resource/BoardFeedResource/get?' + searchParams.toString();
  const options = {
    method: 'GET',
    headers: {
      ...
      // 'x-pinterest-source-url': board_url,
  ...
  const image = {
    ...
    color,
    link: data.resource_response.data[index].link,
  };
...
export default function Home({ image }: InferGetServerSidePropsType<typeof getServerSideProps>) {
  ...
  const clickable = Boolean(Number(searchParams?.get('clickable')))
  console.log({ clickable, searchParams })

  const clickHandler = useCallback(() => {
    if (!image.link || !clickable) {
      return;
    }
